...
function OfficeFilterList() {
  ...
  function fetchData() {
    fetch('http://192.168.100.96:8888/tenantpay/prop/shop/admin_list.php')
  ...
  useEffect(() => {
    const filteredList = MY_LIST.filter((item) => {
      let all_str =
        `${item.id} ${item.shop_name} ${item.building_name} ${item.address} ${item.location} ${item.owner_email}`.toLowerCase()
  ...
  return (
    <div>
      ...
      {/* <div>
        <h6>
          Selected Item : {selected.id}) {selected.shop_name} , {selected.building_name} ,
          {selected.location}
        </h6>
      </div> */}
        ...
        <CTableBody>
          {filterlist.map((item, key) => (
            <CTableRow v-for="item in tableItems" key={key}>
              ...
              <CTableDataCell>
                <div>
                  <Link to={`/properties/shop/details:${item.id}`} className="btn btn-dark btn-xs">
                    Details
                  </Link>
                </div>
              </CTableDataCell>
